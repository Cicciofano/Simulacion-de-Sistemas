...
porcentaje <- c()
#for(p in c(.993, .994, .995, .996, .997)){

for(rep in c(1,2,3,4,5)){
  
modelos[modelos=='n'] <- 0.994
modelos[modelos=='g'] <- 0.94 
modelos[modelos=='b'] <- 0.0015 
...
#}
stopCluster(cluster)
print(max(porcentaje))

#png("p12t5000.png")
#boxplot(Secuencial, Paralelo, ylab="Segundos", names = c("Secuencial", "Paralelo"))
#graphics.off()
plot(porcentaje, type = "lines")

#png("p12negro.png")
#boxplot(a1,a2,a3,a4,a5, ylab="Porcentaje", names=c(.993, .994, .995, .996, .997), main="Negro")
#graphics.off()
